...
const state = {
  ...
  selection: null, gesturePreview: null, clipboard: null,
};
  ...
  return `
    ...
    <main class="workspace">
      <aside class="inspector">
        ...
        <section class="control-section actions"><div class="section-label">Selection actions</div><div class="action-row"><button id="undoButton" disabled>Undo</button><button id="copySelection" disabled>Copy</button><button id="pasteSelection" disabled>Paste</button><button id="fillSelection" disabled>Fill</button><button id="deleteSelection" class="danger" disabled>Delete</button><button id="clearLayer">Clear layer</button><button class="danger" id="clearAll">Clear all</button></div><div class="status" id="status"></div></section>
...
function recordHistory() { state.history.push({ blocks: snapshotBlocks(), selection: state.selection ? structuredClone(state.selection) : null }); if (state.history.length > 50) state.history.shift(); updateUndoButton(); }
function updateUndoButton() { $('undoButton').disabled = state.history.length === 0; }
function undo() { const previous = state.history.pop(); if (!previous) return; state.blocks = previous instanceof Map ? previous : previous.blocks; if (!(previous instanceof Map)) state.selection = previous.selection; markGrid(); updateSelectionActions(); updateUndoButton(); setStatus('Undid last action'); }
