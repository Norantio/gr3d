...
const saveLibrary = (creations) => localStorage.setItem(LIBRARY_KEY, JSON.stringify(creations));
  ...
  return `
    ...
    <main class="workspace">
      <aside class="inspector">
        <div class="intro"><span class="eyebrow">BUILD / 01</span><h1>Voxel Foundry</h1><p>Shape a block world one layer at a time.</p></div>
        <section class="control-section"><div class="section-label">Slice plane</div><div class="segmented" id="axisControls"><button data-axis="X">X axis</button><button data-axis="Y" class="active">Y axis</button><button data-axis="Z">Z axis</button></div><p class="help">Choose the direction you are looking through the model.</p></section>
        <section class="control-section"><div class="section-heading"><div class="section-label">Current layer</div><span class="layer-readout" id="layerReadout">Y = 0</span></div><div class="layer-controls"><button class="step-button" id="layerDown" aria-label="Previous layer">−</button><div class="layer-track"><div class="layer-track-fill" id="layerFill"></div></div><button class="step-button" id="layerUp" aria-label="Next layer">+</button></div><p class="help">Use the arrow keys to move through the build.</p></section>
      ...
      <section class="stage" aria-label="Voxel editor">
        ...
        <div class="preview-panel" id="previewPanel"><div class="preview-heading"><span>Orbit preview</span><button class="icon-button" id="expandPreview" title="Expand preview" aria-label="Expand preview">⤢</button></div><canvas id="previewCanvas"></canvas><div id="emptyPreview" class="empty-preview">Your build will appear here.</div></div>
...
function updateToolButtons() { document.querySelectorAll('[data-tool]').forEach((button) => button.classList.toggle('active', button.dataset.tool === state.tool)); $('shapeControls').hidden = state.tool !== 'shape'; $('toolHelp').textContent = state.tool === 'select' ? 'Click and drag on the grid to select cells.' : 'Right-click any cell to erase it.'; updateSelectionActions(); }
...
function rebuildPreview() { while (previewGroup.children.length) { const child = previewGroup.children.pop(); child.material?.map?.dispose(); child.material?.dispose(); } if (!state.blocks.size) { $('emptyPreview').hidden = false; return; } $('emptyPreview').hidden = true; let minX = Infinity; let maxX = -Infinity; let minY = Infinity; let maxY = -Infinity; let minZ = Infinity; let maxZ = -Infinity; state.blocks.forEach((color, key) => { const [x, y, z] = key.split(',').map(Number); const mesh = new THREE.Mesh(cubeGeometry, new THREE.MeshStandardMaterial({ color, roughness: 0.82 })); mesh.position.set(x, y, z); const edges = new THREE.LineSegments(edgesGeometry, edgeMaterial); edges.position.copy(mesh.position); previewGroup.add(mesh, edges); minX = Math.min(minX, x); maxX = Math.max(maxX, x); minY = Math.min(minY, y); maxY = Math.max(maxY, y); minZ = Math.min(minZ, z); maxZ = Math.max(maxZ, z); }); const centerX = (minX + maxX) / 2; const centerY = (minY + maxY) / 2; const centerZ = (minZ + maxZ) / 2; const axisLength = Math.max(maxX - minX, maxY - minY, maxZ - minZ, 1) * 0.8; const axes = new THREE.AxesHelper(axisLength); previewGroup.add(axes); const labelSize = Math.max(axisLength * 0.22, 0.35); previewGroup.add(makeAxisLabel('X', '#d85c4a', new THREE.Vector3(axisLength + labelSize * 0.35, 0, 0), labelSize)); previewGroup.add(makeAxisLabel('Y', '#438c63', new THREE.Vector3(0, axisLength + labelSize * 0.35, 0), labelSize)); previewGroup.add(makeAxisLabel('Z', '#4b78b4', new THREE.Vector3(0, 0, axisLength + labelSize * 0.35), labelSize)); const sliceSize = Math.max(maxX - minX, maxY - minY, maxZ - minZ, 1) + 2; const slice = new THREE.Mesh(new THREE.PlaneGeometry(sliceSize, sliceSize), new THREE.MeshBasicMaterial({ color: 0x7a8085, transparent: true, opacity: 0.16, side: THREE.DoubleSide, depthWrite: false })); slice.renderOrder = -1; if (state.axis === 'Y') { slice.rotation.x = -Math.PI / 2; slice.position.set(0, state.layer - centerY, 0); } else if (state.axis === 'X') { slice.rotation.y = Math.PI / 2; slice.position.set(state.layer - centerX, 0, 0); } else { slice.position.set(0, 0, state.layer - centerZ); } previewGroup.add(slice); previewGroup.position.set(-centerX, -centerY, -centerZ); previewDistance = (Math.max(maxX - minX, maxY - minY, maxZ - minZ, 1) + 2) * 1.6; updateCamera(); }
function resizePreview() { const size = previewCanvas.parentElement.clientWidth; pRenderer.setSize(size, size, false); pCamera.aspect = 1; pCamera.updateProjectionMatrix(); }
...
seed(); renderPalette(); updateLabels(); updateToolButtons(); updateShapeButtons(); updateUndoButton(); renderHome(); new ResizeObserver(() => { resizeGrid(); resizePreview(); }).observe(stage); resizeGrid(); resizePreview(); requestAnimationFrame(() => { resetView(); state.previewDirty = true; }); loop();
