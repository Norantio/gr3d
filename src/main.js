...
$('previewResizeHandle').addEventListener('pointerdown', (event) => {
  ...
  previewResize = { x: event.clientX, y: event.clientY, size: panel.getBoundingClientRect().width };
  panel.classList.add('resizing');
});
$('previewResizeHandle').addEventListener('pointermove', (event) => {
  if (!previewResize) return;
  const maxSize = Math.max(300, Math.min(stage.clientWidth - 36, stage.clientHeight - 36));
  const delta = Math.max(previewResize.x - event.clientX, event.clientY - previewResize.y);
  $('previewPanel').style.width = `${clamp(previewResize.size + delta, 300, maxSize)}px`;
  resizePreview();
});
