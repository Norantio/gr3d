...
const state = {
  ...
  scale: 28, offsetX: 0, offsetY: 0, gridDirty: true, previewDirty: true, creationId: null, creationName: '', history: [], shape: 'square', shapeFill: false,
  selection: null, gesturePreview: null, clipboard: null, redoHistory: [], extrudeDepth: 1,
};
...
function appTemplate() {
  return `
    ...
    <header class="topbar">
      <div class="brand"><button class="back-button" id="homeButton">← Library</button><button class="editor-title-button" id="editorTitleButton" title="Rename creation"><span id="editorTitle">Untitled creation</span><span class="editor-title-icon">✎</span></button></div>
    ...
    <main class="workspace">
      <aside class="inspector">
        ...
        <section class="control-section"><div class="section-label">Tools</div><div id="toolControls"><div class="tool-group-label">Draw</div><div class="tool-grid"><button class="active" data-tool="paint" title="Place or remove one voxel">Paint</button><button data-tool="erase" title="Remove one voxel">Erase</button><button data-tool="line" title="Drag a straight line">Line</button><button data-tool="shape" title="Drag a rectangle or ellipse">Shape</button><button data-tool="fill" title="Fill a connected region">Fill</button></div><div class="tool-group-label">Inspect</div><div class="tool-grid"><button data-tool="select" title="Drag to select; drag selection to move">Select</button><button data-tool="pick" title="Pick a material from the grid">Pick</button></div></div><div class="shape-controls" id="shapeControls"><div class="segmented shape-types"><button data-shape="circle">Ellipse</button><button class="active" data-shape="square">Rectangle</button></div><div class="segmented shape-fill"><button class="active" data-shape-fill="outline">Outline</button><button data-shape-fill="filled">Filled</button></div></div><p class="help" id="toolHelp">Right-click any cell to erase it.</p></section>
        <section class="control-section actions"><div class="section-label">Selection actions</div><div class="action-row"><div class="history-controls"><button class="history-button" id="undoButton" title="Undo" aria-label="Undo" disabled>↶</button><button class="history-button" id="redoButton" title="Redo" aria-label="Redo" disabled>↷</button></div><button id="copySelection" disabled>Copy</button><button id="pasteSelection" disabled>Paste</button><button id="fillSelection" disabled>Fill</button><button id="deleteSelection" class="danger" disabled>Delete</button><button id="clearLayer">Clear layer</button><button class="danger" id="clearAll">Clear all</button></div><div class="extrude-controls"><label for="extrudeDepth">Extrude</label><input id="extrudeDepth" type="number" min="1" max="64" value="1" aria-label="Extrusion layers" /><button id="extrudeNegative" title="Extrude toward lower Y" aria-label="Extrude toward lower Y" disabled>−Y</button><button id="extrudePositive" title="Extrude toward higher Y" aria-label="Extrude toward higher Y" disabled>+Y</button></div><div class="status" id="status"></div></section>
...
function loadBlocks(blocks) { state.blocks = new Map((blocks || []).map((block) => [keyOf(block.x | 0, block.y | 0, block.z | 0), block.color || '#5b8a3c'])); state.history = []; state.redoHistory = []; state.selection = null; state.layer = 0; resetView(); markGrid(); updateHistoryButtons(); }
...
function createNewCreation() { state.blocks = new Map(); seed(); state.history = []; state.redoHistory = []; state.selection = null; state.creationId = crypto.randomUUID(); state.creationName = 'Untitled creation'; state.layer = 0; resetView(); markGrid(); updateHistoryButtons(); showEditor(); }
...
function snapshotEditor() { return { blocks: snapshotBlocks(), selection: state.selection ? structuredClone(state.selection) : null }; }
function restoreEditor(snapshot) { state.blocks = snapshot.blocks; state.selection = snapshot.selection; markGrid(); updateSelectionActions(); }
function recordHistory() { state.history.push(snapshotEditor()); if (state.history.length > 50) state.history.shift(); state.redoHistory = []; updateHistoryButtons(); }
function updateHistoryButtons() { $('undoButton').disabled = state.history.length === 0; $('redoButton').disabled = state.redoHistory.length === 0; }
function undo() { const previous = state.history.pop(); if (!previous) return; state.redoHistory.push(snapshotEditor()); restoreEditor(previous); updateHistoryButtons(); setStatus('Undid last action'); }
function redo() { const next = state.redoHistory.pop(); if (!next) return; state.history.push(snapshotEditor()); restoreEditor(next); updateHistoryButtons(); setStatus('Redid last action'); }
...
function updateLabels() {
  ...
  $('extrudeNegative').textContent = `−${state.axis}`;
  $('extrudeNegative').title = `Extrude toward lower ${state.axis}`;
  $('extrudeNegative').ariaLabel = `Extrude toward lower ${state.axis}`;
  $('extrudePositive').textContent = `+${state.axis}`;
  $('extrudePositive').title = `Extrude toward higher ${state.axis}`;
  $('extrudePositive').ariaLabel = `Extrude toward higher ${state.axis}`;
}
...
function updateToolButtons() { document.querySelectorAll('[data-tool]').forEach((button) => button.classList.toggle('active', button.dataset.tool === state.tool)); $('shapeControls').hidden = state.tool !== 'shape'; $('toolHelp').hidden = ['select', 'shape', 'line'].includes(state.tool); $('toolHelp').textContent = state.tool === 'fill' ? 'Fill enclosed empty space or a connected material.' : state.tool === 'pick' ? 'Choose a material from the current layer.' : 'Right-click any cell to erase it.'; updateSelectionActions(); }
function updateShapeButtons() { document.querySelectorAll('[data-shape]').forEach((button) => button.classList.toggle('active', button.dataset.shape === state.shape)); document.querySelectorAll('[data-shape-fill]').forEach((button) => button.classList.toggle('active', (button.dataset.shapeFill === 'filled') === state.shapeFill)); }
...
function updateSelectionActions() { const disabled = !state.selection; const empty = disabled || selectedKeys().length === 0; $('fillSelection').disabled = disabled; $('deleteSelection').disabled = empty; $('copySelection').disabled = empty; $('pasteSelection').disabled = !state.clipboard; $('extrudeNegative').disabled = empty; $('extrudePositive').disabled = empty; }
...
function copySelected() { const bounds = selectionBounds(); if (!bounds) return; const cells = selectedKeys().map((key) => { const [x, y, z] = key.split(',').map(Number); const cell = decompose(x, y, z, state.axis); return { col: cell.col - bounds.left, row: cell.row - bounds.top, color: state.blocks.get(key) }; }); if (!cells.length) return; state.clipboard = { width: bounds.right - bounds.left + 1, height: bounds.bottom - bounds.top + 1, cells }; updateSelectionActions(); setStatus(`Copied ${cells.length} ${cells.length === 1 ? 'voxel' : 'voxels'}`); }
function pasteSelected() { if (!state.clipboard) return; const bounds = selectionBounds(); const left = (bounds?.left ?? 0) + 1; const top = (bounds?.top ?? 0) + 1; recordHistory(); state.clipboard.cells.forEach(({ col, row, color }) => { const world = compose(left + col, top + row, state.layer, state.axis); state.blocks.set(keyOf(world.x, world.y, world.z), color); }); state.selection = { start: { col: left, row: top }, end: { col: left + state.clipboard.width - 1, row: top + state.clipboard.height - 1 } }; markGrid(); updateSelectionActions(); setStatus(`Pasted ${state.clipboard.cells.length} ${state.clipboard.cells.length === 1 ? 'voxel' : 'voxels'}`); }
...
function extrudeSelection(direction) { const source = selectedKeys().map((key) => { const [x, y, z] = key.split(',').map(Number); const cell = decompose(x, y, z, state.axis); return { col: cell.col, row: cell.row, color: state.blocks.get(key) }; }); if (!source.length) return; const depth = clamp(Number(state.extrudeDepth) || 1, 1, 64); let startDistance = 1; while (source.every(({ col, row, color }) => { const world = compose(col, row, state.layer + direction * startDistance, state.axis); return state.blocks.get(keyOf(world.x, world.y, world.z)) === color; })) startDistance += 1; const targets = new Map(); source.forEach(({ col, row, color }) => { for (let offset = 0; offset < depth; offset += 1) { const world = compose(col, row, state.layer + direction * (startDistance + offset), state.axis); targets.set(keyOf(world.x, world.y, world.z), color); } }); recordHistory(); targets.forEach((color, key) => state.blocks.set(key, color)); markGrid(); setStatus(`Extruded ${source.length} ${source.length === 1 ? 'voxel' : 'voxels'} through ${depth} more ${depth === 1 ? 'layer' : 'layers'}`); }
...
function drawGrid() {
  ...
  if (state.gesturePreview) { grid.fillStyle = `${state.color}99`; state.gesturePreview.forEach((cellKey) => { const [col, row] = cellKey.split(',').map(Number); grid.fillRect(col * state.scale + state.offsetX + 2, row * state.scale + state.offsetY + 2, state.scale - 4, state.scale - 4); }); }
...
function lineCells(start, end) { const cells = new Set(); let col = start.col; let row = start.row; const deltaCol = Math.abs(end.col - col); const deltaRow = Math.abs(end.row - row); const stepCol = col < end.col ? 1 : -1; const stepRow = row < end.row ? 1 : -1; let error = deltaCol - deltaRow; while (true) { cells.add(`${col},${row}`); if (col === end.col && row === end.row) break; const doubled = error * 2; if (doubled > -deltaRow) { error -= deltaRow; col += stepCol; } if (doubled < deltaCol) { error += deltaCol; row += stepRow; } } return cells; }
function shapeCells(start, end) { const left = Math.min(start.col, end.col); const right = Math.max(start.col, end.col); const top = Math.min(start.row, end.row); const bottom = Math.max(start.row, end.row); const filled = new Set(); if (state.shape === 'square') { for (let row = top; row <= bottom; row += 1) for (let col = left; col <= right; col += 1) filled.add(`${col},${row}`); } else { const radiusX = Math.max((right - left + 1) / 2, 0.5); const radiusY = Math.max((bottom - top + 1) / 2, 0.5); const centerX = (left + right) / 2; const centerY = (top + bottom) / 2; for (let row = top; row <= bottom; row += 1) for (let col = left; col <= right; col += 1) if (((col - centerX) / radiusX) ** 2 + ((row - centerY) / radiusY) ** 2 <= 1) filled.add(`${col},${row}`); }
  if (state.shapeFill) return filled; const outline = new Set(); filled.forEach((cellKey) => { const [col, row] = cellKey.split(',').map(Number); if ([[1, 0], [-1, 0], [0, 1], [0, -1]].some(([dx, dy]) => !filled.has(`${col + dx},${row + dy}`))) outline.add(cellKey); }); return outline; }
function previewCells(start, end) { return state.tool === 'line' ? lineCells(start, end) : shapeCells(start, end); }
function applyCells(cells, erase = false) { if (!cells?.size) return; recordHistory(); cells.forEach((cellKey) => { const [col, row] = cellKey.split(',').map(Number); const world = compose(col, row, state.layer, state.axis); const key = keyOf(world.x, world.y, world.z); if (erase) state.blocks.delete(key); else state.blocks.set(key, state.color); }); markGrid(); }
function pickMaterial(cell) { const world = compose(cell.col, cell.row, state.layer, state.axis); const color = state.blocks.get(keyOf(world.x, world.y, world.z)); if (!color) { setStatus('No material in that cell'); return; } state.color = color; state.tool = 'paint'; $('customColor').value = color; renderPalette(); updateToolButtons(); updateLabels(); setStatus('Material picked'); }
function floodFill(cell) { const world = compose(cell.col, cell.row, state.layer, state.axis); const targetColor = state.blocks.get(keyOf(world.x, world.y, world.z)); if (targetColor === state.color) return; const layerCells = new Map(); state.blocks.forEach((color, key) => { const [x, y, z] = key.split(',').map(Number); const current = decompose(x, y, z, state.axis); if (current.layer === state.layer) layerCells.set(`${current.col},${current.row}`, color); }); if (!layerCells.size) { setStatus('Draw a boundary before filling empty space'); return; }
  let minCol = Infinity; let maxCol = -Infinity; let minRow = Infinity; let maxRow = -Infinity; layerCells.forEach((color, cellKey) => { const [col, row] = cellKey.split(',').map(Number); minCol = Math.min(minCol, col); maxCol = Math.max(maxCol, col); minRow = Math.min(minRow, row); maxRow = Math.max(maxRow, row); }); minCol -= 1; maxCol += 1; minRow -= 1; maxRow += 1; if (!targetColor && (cell.col < minCol || cell.col > maxCol || cell.row < minRow || cell.row > maxRow)) { setStatus('Empty area must be enclosed'); return; }
  const pending = [cell]; const visited = new Set(); let open = false; while (pending.length) { const current = pending.pop(); const cellKey = `${current.col},${current.row}`; if (visited.has(cellKey) || layerCells.get(cellKey) !== targetColor) continue; visited.add(cellKey); if (!targetColor && (current.col === minCol || current.col === maxCol || current.row === minRow || current.row === maxRow)) open = true; [[1, 0], [-1, 0], [0, 1], [0, -1]].forEach(([dx, dy]) => { const col = current.col + dx; const row = current.row + dy; if (col >= minCol && col <= maxCol && row >= minRow && row <= maxRow) pending.push({ col, row }); }); }
  if (open) { setStatus('Empty area must be enclosed'); return; } applyCells(visited); }
...
$('toolControls').addEventListener('click', (event) => { const button = event.target.closest('[data-tool]'); if (!button) return; state.tool = button.dataset.tool; state.gesturePreview = null; if (state.tool !== 'select') state.selection = null; updateToolButtons(); markGrid(); }); $('shapeControls').addEventListener('click', (event) => { const shapeButton = event.target.closest('[data-shape]'); const fillButton = event.target.closest('[data-shape-fill]'); if (shapeButton) state.shape = shapeButton.dataset.shape; if (fillButton) state.shapeFill = fillButton.dataset.shapeFill === 'filled'; updateShapeButtons(); });
$('undoButton').addEventListener('click', undo);
$('redoButton').addEventListener('click', redo);
$('copySelection').addEventListener('click', copySelected); $('pasteSelection').addEventListener('click', pasteSelected); $('fillSelection').addEventListener('click', fillSelectionArea); $('deleteSelection').addEventListener('click', deleteSelected);
$('extrudeDepth').addEventListener('input', (event) => { state.extrudeDepth = clamp(Number(event.target.value) || 1, 1, 64); event.target.value = state.extrudeDepth; }); $('extrudeNegative').addEventListener('click', () => extrudeSelection(-1)); $('extrudePositive').addEventListener('click', () => extrudeSelection(1));
$('layerUp').addEventListener('click', () => { state.layer += 1; markGrid(); }); $('layerDown').addEventListener('click', () => { state.layer -= 1; markGrid(); }); $('resetView').addEventListener('click', resetView); $('clearLayer').addEventListener('click', () => { const previous = snapshotEditor(); [...state.blocks.keys()].forEach((key) => { const [x, y, z] = key.split(',').map(Number); if (decompose(x, y, z, state.axis).layer === state.layer) state.blocks.delete(key); }); if (state.blocks.size !== previous.blocks.size) { state.history.push(previous); if (state.history.length > 50) state.history.shift(); state.redoHistory = []; updateHistoryButtons(); } markGrid(); setStatus('Layer cleared'); }); $('clearAll').addEventListener('click', () => { if (!state.blocks.size) return; recordHistory(); state.blocks.clear(); markGrid(); setStatus('Build cleared'); }); $('exportButton').addEventListener('click', exportBuild); $('importButton').addEventListener('click', () => $('fileInput').click());
...
gridCanvas.addEventListener('pointerdown', (event) => {
  ...
  drag = { x: event.clientX, y: event.clientY, cell, offsetX: state.offsetX, offsetY: state.offsetY, button: event.button, moved: false, kind: 'pan' };
  if (state.tool === 'select') {
    const bounds = selectionBounds();
    const insideSelection = bounds && cell.col >= bounds.left && cell.col <= bounds.right && cell.row >= bounds.top && cell.row <= bounds.bottom;
    if (insideSelection) { drag.kind = 'move-selection'; drag.selection = structuredClone(state.selection); }
    else { drag.kind = 'select'; state.selection = { start: { ...cell }, end: { ...cell } }; updateSelectionActions(); markGrid(); }
  } else if (state.tool === 'line' || state.tool === 'shape') {
    drag.kind = 'draw';
    state.gesturePreview = previewCells(cell, cell);
    state.gridDirty = true;
  }
});
gridCanvas.addEventListener('pointermove', (event) => {
  const cell = cellAt(event.clientX, event.clientY);
  const world = compose(cell.col, cell.row, state.layer, state.axis);
  $('coordinateReadout').textContent = `x ${world.x} · y ${world.y} · z ${world.z}`;
  if (!drag) return;
  ...
  if (drag.kind === 'select') {
    ...
    const deltaCol = cell.col - drag.cell.col;
    const deltaRow = cell.row - drag.cell.row;
    state.selection = { start: { col: drag.selection.start.col + deltaCol, row: drag.selection.start.row + deltaRow }, end: { col: drag.selection.end.col + deltaCol, row: drag.selection.end.row + deltaRow } };
    state.gridDirty = true;
  } else if (drag.kind === 'draw') {
    state.gesturePreview = previewCells(drag.cell, cell);
    state.gridDirty = true;
  } else if (drag.moved) {
    state.offsetX = drag.offsetX + dx;
    state.offsetY = drag.offsetY + dy;
    state.gridDirty = true;
  }
});
gridCanvas.addEventListener('pointerup', (event) => {
  if (!drag) return;
  const cell = cellAt(event.clientX, event.clientY);
  if (drag.kind === 'draw') {
    const cells = previewCells(drag.cell, cell);
    state.gesturePreview = null;
    applyCells(cells, drag.button === 2);
  } else if (drag.kind === 'move-selection') {
    const deltaCol = cell.col - drag.cell.col;
    const deltaRow = cell.row - drag.cell.row;
    state.selection = drag.selection;
    if (deltaCol || deltaRow) moveSelection(deltaCol, deltaRow);
    else state.gridDirty = true;
  } else if (drag.kind === 'pan' && !drag.moved) {
    if (state.tool === 'fill' && event.button !== 2) floodFill(cell);
    else if (state.tool === 'pick' && event.button !== 2) pickMaterial(cell);
    else paint(event.clientX, event.clientY, drag.button === 2);
  }
  drag = null;
});
gridCanvas.addEventListener('pointercancel', () => { state.gesturePreview = null; drag = null; state.gridDirty = true; }); gridCanvas.addEventListener('contextmenu', (event) => event.preventDefault()); stage.addEventListener('wheel', (event) => { event.preventDefault(); const rect = gridCanvas.getBoundingClientRect(); zoomAt(event.clientX - rect.left, event.clientY - rect.top, event.deltaY < 0 ? 1.12 : 1 / 1.12); }, { passive: false });
...
seed(); renderPalette(); updateLabels(); updateToolButtons(); updateShapeButtons(); updateHistoryButtons(); renderHome(); syncLibrary(); new ResizeObserver(resizeGrid).observe(stage); new ResizeObserver(resizePreview).observe($('previewPanel')); resizeGrid(); resizePreview(); requestAnimationFrame(() => { resetView(); state.previewDirty = true; }); loop();
