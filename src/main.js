...
function extrudeSelection(direction) { const source = selectedKeys().map((key) => { const [x, y, z] = key.split(',').map(Number); const cell = decompose(x, y, z, state.axis); return { col: cell.col, row: cell.row, color: state.blocks.get(key) }; }); if (!source.length) return; const depth = clamp(Number(state.extrudeDepth) || 1, 1, 64); const targets = new Map(); source.forEach(({ col, row, color }) => { for (let distance = 1; distance <= depth; distance += 1) { const world = compose(col, row, state.layer + direction * distance, state.axis); targets.set(keyOf(world.x, world.y, world.z), color); } }); if ([...targets].every(([key, color]) => state.blocks.get(key) === color)) { setStatus('Extrusion already exists'); return; } recordHistory(); targets.forEach((color, key) => state.blocks.set(key, color)); markGrid(); setStatus(`Extruded ${source.length} ${source.length === 1 ? 'voxel' : 'voxels'} through ${depth} ${depth === 1 ? 'layer' : 'layers'}`); }
